...
libSDL:
	cd libSDL; unset PKG_CONFIG_LIBDIR; ./configure --host=$(PREFIX) --prefix="$(ROOT)" --enable-shared=no --enable-assertions=release --enable-render=no --enable-joystick=no --enable-haptic=no --enable-power=no --enable-loadso=no --enable-ssemath --enable-sse2 --disable-alsatest --enable-diskaudio=no --enable-dummyaudio=no --enable-video-dummy=no --enable-libudev=no --enable-dbus=no --enable-input-tslib=no --enable-render-d3d=no
